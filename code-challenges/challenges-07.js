...
const objLat = (obj) => {
    let firstName = obj.firstName.slice(0,1).toUpperCase() + obj.firstName.slice(1)
    let lastName = obj.lastName.slice(0,1).toUpperCase() + obj.lastName.slice(1)
    ...
    for (let i = 0; i < data.grades.length; i++) {
        for (let j=0; j < data.grades[i].classes.length; j++) {
            ...
            for (let k = 0; k < data.grades[i].classes[j].classScores.length; k++) {
                ...
                // let avg = Math.floor(sum /data.grades[i].classes[j].classScores.length);
                // data.grades[i].classes[j].avg = avg;
