...
const recursionPattern = (int1, int2, arr= [int1,int1]) => {
     let total;
     total = int1-int2
    function count(total) {
        if (total>0 ){
        arr.splice(Math.round(arr.length/2),0,total)
        arr.splice(Math.round(arr.length/2),0,total)
...
const filterLinks = (str) => {
    const regexUrl = /www.\w+.(com|org|net)/ig

    return str.match(regexUrl)[0]
}
// -------------------------------------------------------------------------------------------------------
...
const isPalindrome = (str) => {
    if(str === " ") return true 
        ...
        let regex = /[A-Za-z]/ig
        let converted = str.match(regex)
        let string = ""
        for (let i =0; i<converted.length; i++) {
            string+= converted[i]
        }
        let lowerCase = string.toLocaleLowerCase()
        let flipped = lowerCase.split('').reverse().join('')
    
        if (lowerCase === flipped ) {
            return true
        }
         return false
    }
